...
class EventKitManager: ObservableObject, CalendarManaging {
    ...
    private func handleAccessResult(granted: Bool, error: Error?) {
        ...
        if let error = error {
            ...
            AppLogger.eventKit.error("EventKit access error: \(error.localizedDescription)")
        }
    }
    ...
    func calendar(matchingID id: String) -> EKCalendar? {
    ...
    func addEvent(_ parsedEvent: ParsedEvent, to targetCalendar: EKCalendar? = nil) throws {
        guard hasAccess else {
            throw EventKitError.noAccess
        }

        guard let targetCalendar = targetCalendar ?? writableCalendars.first else {
