    ...
    var body: some View {
        ZStack {
            Color(nsColor: .windowBackgroundColor)
                .opacity(appState.enableLiquidGlass ? 0.72 : 1.0)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 14)
                    .strokeBorder(Color.white.opacity(appState.enableLiquidGlass ? 0.2 : 0.08), lineWidth: 1)
                    .padding(14)
            )
        }
        ...
        Form {
            Section("Window") {
                HStack {
                    Text("Always on Top")
                    Spacer()
                    Toggle("", isOn: $appState.windowFloating)
                }
                HStack {
                    Text("Center Window")
                    Spacer()
                    Toggle("", isOn: $appState.windowCentered)
                }
            }

            Section("Appearance") {
                Toggle("Liquid Glass Effect", isOn: $appState.enableLiquidGlass)
            }
            ...
            Section("Event Defaults") {
                HStack {
                    ...
                    Spacer()
                    TextField("", value: $appState.defaultDuration, format: .number)
                        .frame(width: 60)
                ...
                HStack {
                    ...
                    Spacer()
                    TextField("", value: $appState.defaultReminder, format: .number)
                        .frame(width: 60)
                        .textFieldStyle(.roundedBorder)
                    Text("minutes before")
                }
                ...
                if !eventKitManager.writableCalendars.isEmpty {
                    Section("Writable Calendars") {
                        ForEach(eventKitManager.writableCalendars, id: \.calendarIdentifier) { calendar in
                            CalendarRowView(
                                calendar: calendar,
                                isSelected: appState.selectedCalendarID == calendar.calendarIdentifier,
                                isHovered: hoveredCalendar?.calendarIdentifier == calendar.calendarIdentifier,
                                isReadOnly: false
                            ) {
                                appState.selectedCalendarID = calendar.calendarIdentifier
                            }
                            .onHover { isHovering in
